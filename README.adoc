...
== Prerequisites
. VirtualBox: https://www.virtualbox.org/wiki/Downloads[Install Instructions]
. Vagrant: Install Instructions](http://concourse.ci/vagrant.html[Install Instructions]
. Fly CLI: Install Instructions](http://concourse.ci/hello-world.html[Install Instructions]
. PCF DEV: Install Instructions](https://docs.pivotal.io/pcf-dev/#installing[Install Instructions]
. GitHub account 

== Steps
. Update manifest file to:
`manifest.yml`
[source,yaml]
----
...
. Create a GitHub repository: [Create GitHub Repository Instructions](https://help.github.com/articles/creating-a-new-repository/)

. Init, add, commit, remote, and push code to Repo
	```
...
. Create a `ci` folder under the root project directory

. Create a `tasks` folder under `ci` 

. Create `ci/pipeline.yml` and update GitHub uri with your GitHub account
`ci/pipeline.yml`
[source,yaml]
----
...
. Create `ci/pipeline_params.yml` file and input your info
...
. Create `ci/tasks/package.sh` file
`ci/tasks/package.sh`
[source,sh]
----
...
. Directory structure after adding above folders and files
...
. Set the pipeline up with the following fly command:
----
...
. Unpause the pipeline with the following fly command:
----
	- `fly -t lite unpause-pipeline -p gs-rest-service-complete `
----

. Validate that the concourse package job completed successfully through the browser: http://192.168.100.4:8080/
