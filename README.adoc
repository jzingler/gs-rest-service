...
== Bonus: Build a concourse pipeline and Deploy to PCF Dev

== Prerequisites
1. VirtualBox: https://www.virtualbox.org/wiki/Downloads[Install Instructions]

2. Vagrant: Install Instructions](http://concourse.ci/vagrant.html[Install Instructions]

3. Fly CLI: Install Instructions](http://concourse.ci/hello-world.html[Install Instructions]

4. PCF DEV: Install Instructions](https://docs.pivotal.io/pcf-dev/#installing[Install Instructions]

5. GitHub account 

== Steps
1. Update manifest file to:
`manifest.yml`
[source,yaml]
----
include::complete/manifest.yml]
----

2. Create a GitHub repository: [Create GitHub Repository Instructions](https://help.github.com/articles/creating-a-new-repository/)

3. Init, add, commit, remote, and push code to Repo
	```
...
4. Create a `ci` folder under the root project directory

5. Create a `tasks` folder under `ci` 

6. Create `ci/pipeline.yml` and update GitHub uri with your GitHub account
	`ci/pipeline.yml`
	[source,yaml]
	----
	include::complete/ci/pipeline.yml]
	----

7. Create `ci/pipeline_params.yml` file and input your info
	`ci/pipeline_params.yml`
	[source,yaml]
	----
	include::complete/ci/pipeline_params.yml]
	----

8. Create `ci/tasks/package.yml`:
	`ci/tasks/package.yml`
	[source,yaml]
	----
	include::complete/ci/tasks/package.yml]
	----

9. Create `ci/tasks/package.sh` file
	`ci/tasks/package.sh`
	[source,sh]
	----
	include::complete/ci/tasks/package.sh]
	----

10. Directory structure after adding above folders and files
...
11. Stage, commit, and push updates: 
----
git add . && git commit -am "create ci pipeline files" && git push
----

12. Login to concourse via FLY: 
----
fly -t lite login -c http://192.168.100.4:8080/
----

13. Set the pipeline up with the following fly command:
----
fly -t lite set-pipeline -p gs-rest-service-concourse -c ci/pipeline.yml --non-interactive --load-vars-from ci/pipeline_params_template.yml
----

14. Unpause the pipeline with the following fly command:
----
fly -t lite unpause-pipeline -p gs-rest-service-complete
----

15. Validate that the concourse package job completed successfully through the browser: http://192.168.100.4:8080/
